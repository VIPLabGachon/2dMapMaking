...
def run_conversion():
    ...
    print(os.getcwd())
    if filepath:
        subprocess.run(["python", "coordinateConversion.py", filepath])

def run_filtering():
    subprocess.run(["python", "pointcloudFiltering.py"])
...
def generate_top_view_images():
    ...
    for pcd_file in pcd_files:
        ...
        image_size = 400  # Increase this size as needed
        ...
        # Display the image horizontally with a larger size
        display_image(img, pcd_file, image_frame)

def display_image(img, pcd_file, parent_frame):
    # Convert the OpenCV image to a PIL image
    image = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    image.thumbnail((3000, 3000))  # Resize the image for display (adjust as needed)
...
def save_selected_image(img, pcd_file):
    ...
    if save_path:
        ...
        Image.fromarray(img).save(save_path, dpi=(720, 720))
...
conv_button = tk.Button(root, text="convert file", command=run_conversion)
conv_button.pack()

p4_button = tk.Button(root, text="Filtering and Generate Images", command=run_filtering)
p4_button.pack()
